...
class SecurityManagedRfcService:
    ...
    def ejecutar(self, context: dict) -> dict:
        ...
        if action == "list":
            return self._list(url, key, user_id)
        if action == "delete":
            return self._delete(context, url, key, user_id)
        return {"ok": False, "error": f"action inválido: {action}. Usa create|list|delete"}
    ...
    def _create(self, context: dict, url: str, key: str, user_id: str) -> dict:
        ...
        if context.get("dry_run", True):
            return {"ok": True, "message": "dry_run", "data": {"rfc": rfc, "user_id": user_id}}
        ...
        return {
            ...
            "data":    {"managed_rfc_id": row.get("id"), "rfc": rfc, "label": label},
        }

    def _list(self, url: str, key: str, user_id: str) -> dict:
        qs   = f"?owner_user_id=eq.{urllib.parse.quote(user_id)}&select=id,rfc,label,created_at&order=created_at.asc"
