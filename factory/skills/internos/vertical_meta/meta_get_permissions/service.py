"""Service for meta_get_permissions - calls Meta Graph API /me/permissions."""
...
class MetaGetPermissionsService:
        ...
        if dry_run:
            return {
                ...
                "data": {
                    ...
                    "path": path,
                    "params": {"access_token": "***"},
                    "url": self._build_url(graph_version, path, {"access_token": "***"}),
                },
            }

        try:
            result = self._get(url)
            return {"ok": True, "data": {"permissions": result.get("data", []), "paging": result.get("paging")}}
