...
import { companyName, findUserByEmail, listCompanies, listGrants, logisticsGrants } from "@/lib/platform";
...
export async function POST(req: NextRequest) {
  ...
  const login = await callFactory("vertical_auth_security/security_user_login", {
    email,
    password,
    ip,
    modulo_code: "logistics",
    dry_run: false
  });
  if (!login.ok) return NextResponse.json({ ok: false, error: "Credenciales incorrectas" }, { status: 401 });
  ...
  const allowed = logisticsGrants(grants);
  if (!allowed.length) return NextResponse.json({ ok: false, error: "Usuario sin acceso a Logistics4All" }, { status: 403 });
  const companies = await listCompanies(Array.from(new Set(allowed.map((grant) => grant.company_id))));
  const preferred = allowed.find((grant) => grant.modulo_code === "logistics") || allowed[0];
