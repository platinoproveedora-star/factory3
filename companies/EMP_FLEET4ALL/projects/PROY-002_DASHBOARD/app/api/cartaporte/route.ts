...
import { cartaporteBuild, cartaporteValidate, pacStamp, cartaporteCancel } from "@/lib/fleet4all";
...
export async function POST(req: NextRequest) {
  ...
  const result =
    action === "validate" ? await cartaporteValidate(body) :
    action === "stamp" ? await pacStamp(body) :
    action === "cancel" ? await cartaporteCancel(body) :
    await cartaporteBuild(body);
