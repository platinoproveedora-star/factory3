...
PATTERNS = [
    ...
    ("schema", re.compile(r"\b(?:[a-z][a-z0-9]*_proy\d{3,}|logplat)\b")),
    ...
    ("secret_name", re.compile(r"SUPABASE_SERVICE_ROLE_KEY|SERVICE_ROLE_KEY|ANTHROPIC_API_KEY|GITHUB_TOKEN|RENDER_API_KEY")),
    ("write_key", re.compile(r"NEXT_PUBLIC_WRITE_KEY|FACTORY_WRITE_KEY|DASHBOARD_KEY")),
]

CODE_EXTENSIONS = {".py", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"}
CONFIG_EXTENSIONS = {".json", ".yaml", ".yml", ".md", ".sql", ".example"}
SKIP_PARTS = {"node_modules", ".next", ".git", "__pycache__", "dist", "build", ".venv", "venv", "out"}
ALLOWED_FILENAMES = {
    "project.json",
    "company.json",
    "modules.json",
...
class FactoryNoHardcodeAuditService:
                    ...
                    findings.append(
                        {
                            ...
                            "line": line_no,
                            "match": match.group(0),
                            "message": self._message(severity, kind),
                        }
                    )
        return findings
    ...
    def _severity(self, path: Path, kind: str, line: str) -> str:
        ...
        if "re.compile" in line:
            return "allowed"
        if kind in {"secret_name", "write_key"} and "requires_env" in line:
            return "allowed"
        if name in ALLOWED_FILENAMES or suffix in CONFIG_EXTENSIONS or "/docs/" in lower:
            return "allowed"
        if kind in {"secret_name", "write_key"}:
            return "warning"
    ...
    def _message(self, severity: str, kind: str) -> str:
        if severity == "blocker":
