...
import { listWarehouses, saveWarehouse } from "@/lib/factu4all";
...
export async function GET() {
  ...
  const result = await listWarehouses(user.company_id);
...
export async function POST(req: NextRequest) {
  ...
  const result = await saveWarehouse(user.company_id, body);
  return NextResponse.json(result, { status: result.ok ? 200 : 502 });
}
