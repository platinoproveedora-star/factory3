...
import { advanceCapture, settlementCalculate, receiptGenerate, settlementHistory } from "@/lib/fleet4all";
...
export async function GET(req: NextRequest) {
  ...
  const result = await settlementHistory({ empresa_id: empresaId, driver_key: s.get("driver_key") || undefined });
...
export async function POST(req: NextRequest) {
  ...
  const result =
    action === "calculate" ? await settlementCalculate(body) :
    action === "receipt" ? await receiptGenerate(body) :
    await advanceCapture(body);
