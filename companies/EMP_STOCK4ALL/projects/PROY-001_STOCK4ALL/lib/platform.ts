...
export async function requireCompanyModuleGrant(userId: string, companyId: string, moduloCode: string) {
...
export async function getTenantSource(companyId: string, moduleCode: string, sourceKey = "default") {
  const qs = new URLSearchParams({
    company_id: `eq.${companyId}`,
    module_code: `eq.${moduleCode}`,
    source_key: `eq.${sourceKey}`,
    enabled: "eq.true",
    select: "id,company_id,module_code,source_key,source_schema,source_project_code,enabled,config",
    limit: "1"
  });
  const rows = await platformFetch<TenantSource[]>(`tenant_sources?${qs.toString()}`);
  return rows[0] || null;
}
