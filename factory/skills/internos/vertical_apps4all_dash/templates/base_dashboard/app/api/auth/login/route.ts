...
import { findUserByEmail, listGrants, listCompanies, logLoginAttempt, companyName } from "@/lib/platform";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

export async function POST(req: NextRequest) {
  const moduleCode = process.env.MODULE_CODE || "";
  if (!moduleCode) {
    return NextResponse.json({ ok: false, error: "MODULE_CODE no configurado" }, { status: 500 });
  }
  ...
  try {
    ...
    const chosen = grants.find((grant) => grant.modulo_code === moduleCode);
    if (!chosen) {
      await logLoginAttempt(email, ip, false);
      return NextResponse.json({ ok: false, error: `Sin acceso a ${moduleCode}` }, { status: 403 });
    }
    ...
    const token = await signSession({
      ...
      company_name: companyName(companies, chosen.company_id),
