...
import { rateManage, quoteBuild, quotePdfSend } from "@/lib/fleet4all";
...
export async function GET(req: NextRequest) {
  ...
  const result = await rateManage({ empresa_id: empresaId, action: "list" });
...
export async function POST(req: NextRequest) {
  ...
  const result =
    action === "rate" ? await rateManage(body) :
    action === "pdf" ? await quotePdfSend(body) :
    await quoteBuild(body);
