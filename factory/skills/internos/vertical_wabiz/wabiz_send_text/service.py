"""Envía mensaje de texto a WhatsApp vía Cloud API y registra en wabiz_messages."""
...
class WabizSendTextService:
    ...
    def ejecutar(self, context: dict) -> dict:
        ...
        if not cfg and not (context.get("access_token") and context.get("phone_number_id")):
        ...
        if not token:
            return {"ok": False, "error": "access_token no disponible"}
        if not phone_number_id:
            return {"ok": False, "error": "phone_number_id no disponible"}

        if context.get("dry_run", True):
            return {"ok": True, "data": {"dry_run": True, "to": to, "body": body}}

        try:
            result = self._send(phone_number_id, to, body, token, graph_version)
            if "error" in result:
                err = result["error"]
                return {"ok": False, "error": err.get("message", str(err))}

            wa_message_id = result.get("messages", [{}])[0].get("id")
            self._log_outbound(empresa_id, to, body, wa_message_id)

            return {"ok": True, "data": {"sent": True, "wa_message_id": wa_message_id, "to": to}}
        except Exception as e:
            return {"ok": False, "error": str(e)}

    def _send(self, phone_number_id: str, to: str, body: str, token: str, version: str) -> dict:
    ...
    def _log_outbound(self, empresa_id: str, to: str, body: str, wa_message_id: str) -> None:
        if not empresa_id:
            return
        try:
            row = {
