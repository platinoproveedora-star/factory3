...
export default async function SettingsPage() {
  ...
  if (!user) return null;
  const portalHref = process.env.NEXT_PUBLIC_APPS4ALL_URL || "http://localhost:3000";
  ...
  return (
    <div className="mx-auto max-w-7xl px-5 py-6">
      <section className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
        ...
        <p className="max-w-xl text-sm leading-6 text-slate-600">Empresa seleccionada, grants y configuración general.</p>
      </section>
      <div className="mt-6 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex gap-2">
          ...
          <Link href="/dashboard" className="rounded-md border border-slate-300 px-3 py-2 text-sm hover:border-steel hover:text-steel">Inicio</Link>
        </div>
      </div>
    </div>
  );
